...
public class DirtyAI2 extends AIModule {
	...
	public int makeMove(final GameStateModule game, int column) {
		// store the move to a arraylist, reducing complexity of evaluation Function
		// (also making things easier to deal with)
	...
	public void interactiveDeepeningMiniMaxDecision(final GameStateModule game, int depth) {
		...
		for (int i = 0; i < numLegalMoves && !terminate; i++) {
			...
			if (result_value_aux >= result_value) {// update result if the new result is greater than the previous
													// calculated max
				column = moves[i];
	...
	public int minValue(final GameStateModule game, int depth, int column) {
		...
		if (depth <= 0 || gameIsOver)
			return evaluationFunction(game, gameIsOver, column);
		// cutoff test
					...
					for (int dx = -1; dx <= 1; dx++) {
						for (int dy = -1; dy <= 1; dy++) {
							...
							if (game.getAt(x + dx, y + dy) == p + 1) {
								...
								// the adjancent point or is empty or it belongs to another player, hence
								// without importance
								continue;
							}
						}
